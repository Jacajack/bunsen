...
br::scene_node br::load_mesh_from_file(const std::string &path)
{
	...
	{
		auto mesh_data = std::make_shared<br::mesh_data>();
	...
	{
		auto process_node_impl = [&](const aiScene *scene, aiNode *n, auto &ref)->br::scene_node
		{
			br::scene_node node;
			...
			for (auto i = 0u; i < n->mNumMeshes; i++)
			{
				...
				node.meshes.push_back(br::mesh{mesh_data, nullptr});
			}
